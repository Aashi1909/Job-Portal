...
const JobDescription = () => {
    ...
    const applyJobHandler = async () => {
        try {
            const res = await axios.get(`${APPLICATION_API_URL}/apply/${jobId}`, {withCredentials:true});
    ...
    useEffect(()=>{
        const fetchSingleJob = async () => {
            try {
                const res = await axios.get(`${JOB_API_URL}/get/${jobId}`,{withCredentials:true});
                if(res.data.success){
        ...
        <div className="max-w-7xl mx-auto my-10 ">
            <div className="flex items-center justify-between ">
                ...
                onClick={isApplied ? null : applyJobHandler}
                    disabled={isApplied}
                    className={`rounded-lg ${isApplied ? 'text-md font-semibold text-white  bg-gray-600 rounded-lg" variant="outline' : 'bg-[#F83002] hover:bg-red-700'}`}>
